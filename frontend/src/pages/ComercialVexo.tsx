...
import { ClipboardList, FileText, FileCheck, Package } from "lucide-react";
import { PageShell } from "@/components/PageShell";
import { ErrorBoundary } from "@/components/ErrorBoundary";
import GeracaoDigitalProposals from "./GeracaoDigitalProposals";
import GeracaoDigitalPackages from "./GeracaoDigitalPackages";
...
export default function ComercialVexo() {
  ...
  return (
    <PageShell
      ...
      subtitle="Suíte Comercial Integrada Vexo OS: Propostas, Módulos & Catálogo, Briefings e Contratos"
    >
      <div className="w-full space-y-6">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <div className="border-b border-slate-200 dark:border-white/10 pb-3 mb-6">
            <TabsList className="flex w-full max-w-3xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/10 h-11 p-1 rounded-xl">
              <TabsTrigger
                value="propostas"
          ...
          <TabsContent value="modulos" className="mt-0 focus-visible:outline-none">
            <ErrorBoundary>
              <GeracaoDigitalPackages isVexoCommercial={true} />
            </ErrorBoundary>
          </TabsContent>
